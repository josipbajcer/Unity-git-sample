...
namespace BehaviorDesigner.Runtime.Tasks.Movement
{
	...
	public class db_ObjectWithinSightRay: Conditional
	{
			...
			for (int i = 0; i < possibleTargets.Length; ++i) {
				...
				if (usePhysics2D) {
					...
					if (target.Value == null) {
						...
						returnedObject.Value = MovementUtility.WithinSight2D (transform, offset.Value, fieldOfViewAngle.Value, viewDistance.Value, target.Value, targetOffset.Value, ignoreLayerMask);
					}
				} else {
					// If the target object is null then determine if there are any objects within sight based on the layer mask
					if (target.Value == null) {
						...
						returnedObject.Value = MovementUtility.WithinSight (transform, offset.Value, fieldOfViewAngle.Value, viewDistance.Value, target.Value, targetOffset.Value, ignoreLayerMask);
					}
				}
